...
export default async function PeoplePage({ searchParams }: { searchParams: Promise<{ status?: string; q?: string; message?: string; error?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f4f5f2] text-[#1d2923]">
    ...
    <div className="mx-auto max-w-7xl p-6 sm:p-10">
      <div className="flex flex-col justify-between gap-4 md:flex-row"><div><p className="text-xs font-bold tracking-[.18em] text-[#9a7b29]">PEOPLE CRM</p><h1 className="mt-2 text-4xl font-semibold">People</h1><p className="mt-2 text-[#69736c]">Members, attenders and visitors in one secure church workspace.</p></div></div>
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[1fr_340px]">
        <section className="rounded-2xl border border-[#e0e3de] bg-white p-5">
          ...
          <div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-[#e7e9e5] text-[#7a847d]"><tr><th className="py-3">Name</th><th>Status</th><th>Contact</th><th>First visit</th></tr></thead><tbody className="divide-y divide-[#eceeea]">{people?.map(person=><tr key={person.id}><td className="py-4 pr-4 font-semibold">{person.first_name} {person.last_name}</td><td className="pr-4"><span className="rounded-full bg-[#f2eee2] px-2.5 py-1 text-xs capitalize text-[#755d20]">{person.status.replaceAll("_"," ")}</span></td><td className="pr-4 text-[#667068]"><div>{person.email ?? "—"}</div><div>{person.phone ?? ""}</div></td><td className="text-[#667068]">{person.first_visit_date ?? "—"}</td></tr>)}</tbody></table>{!people?.length && <div className="py-14 text-center text-[#7a847d]">No people found. Add your first person to begin building the church CRM.</div>}</div>
        </section>
