export type BriefingInput = {
  ...
  requiredServePositions?: number;
  filledServePositions?: number;
  vacantServePositions?: number;
};
...
export function buildMorningBriefing(input: BriefingInput) {
  ...
  if ((input.vacantServePositions ?? 0) > 0) items.push({ level: "urgent", title: `${input.vacantServePositions} Serve position${input.vacantServePositions === 1 ? "" : "s"} still vacant`, detail: `Upcoming services need ${input.requiredServePositions ?? 0} volunteers across planned positions; ${input.filledServePositions ?? 0} are currently filled.`, href: "/serve" });
  ...
  if (!items.length) items.push({ level: "positive", title: "No immediate operational gaps", detail: "Your current people, task and Serve records show no overdue, vacant or unconfirmed actions." });

  const serve = input.requiredServePositions === undefined ? (input.upcomingServeAssignments === undefined ? "" : ` You also have ${input.upcomingServeAssignments} upcoming Serve assignment${input.upcomingServeAssignments === 1 ? "" : "s"}.`) : ` Upcoming services require ${input.requiredServePositions} planned volunteer position${input.requiredServePositions === 1 ? "" : "s"}; ${input.filledServePositions ?? 0} are filled and ${input.vacantServePositions ?? 0} remain vacant.`;
